// js/leaderboard.js — public leaderboard page: top 50 theo điểm/streak.
...
async function paint(el) {
  ...
  const body = !user
    ? '<tr><td colspan="5" class="lb-empty">Đăng nhập để xem bảng xếp hạng cùng bạn bè.</td></tr>'
    : rows.length === 0
    ? '<tr><td colspan="5" class="lb-empty">Chưa có ai trên bảng xếp hạng — hoàn thành bài học đầu tiên để lên hạng!</td></tr>'
    : rows.map((row) => `
